...
erDiagram
    ...
    User ||--o{ Coupon : "owns"
    CouponTemplate ||--o{ Coupon : "issues"
...
| emailVerified | DateTime? | YES | null | 이메일 인증 일시 |
...
|------------|------|---------|-----------|
| Account | 1:N | Account.userId | CASCADE |
| Session | 1:N | Session.userId | CASCADE |
...
### 2.2 Account (소셜 계정 연동)

| 항목 | 내용 |
|------|------|
| **테이블명** | accounts |
| **설명** | NextAuth.js OAuth 프로바이더별 연동 정보 (1 User : N Account) |
...
| type | String | NO | - | oauth, oidc, email 등 |
| provider | String | NO | - | kakao, naver, google 등 |
| providerAccountId | String | NO | - | 프로바이더 고유 ID |
| refresh_token | String? @db.Text | YES | null | 리프레시 토큰 |
| access_token | String? @db.Text | YES | null | 액세스 토큰 |
| expires_at | Int? | YES | null | 토큰 만료 시각 (Unix) |
| token_type | String? | YES | null | bearer 등 |
| scope | String? | YES | null | 권한 범위 |
| id_token | String? @db.Text | YES | null | OIDC ID 토큰 |
| session_state | String? | YES | null | 세션 상태 |

#### 인덱스

| 인덱스명 | 컬럼 | 타입 | 용도 |
|----------|------|------|------|
| accounts_provider_providerAccountId | provider, providerAccountId | UNIQUE | 프로바이더별 계정 조회 |
| accounts_userId | userId | INDEX | 사용자별 연동 계정 |
...
|--------|------|------|--------|------|
| id | String @id @default(cuid()) | NO | cuid | PK |
| sessionToken | String @unique | NO | - | 세션 토큰 |
| userId | String | NO | - | FK → User |
| expires | DateTime | NO | - | 세션 만료 일시 |

---

### 2.4 VerificationToken (인증 토큰)

| 항목 | 내용 |
|------|------|
| **테이블명** | verification_tokens |
| **설명** | 이메일 인증, 비밀번호 재설정 토큰 |

#### 컬럼 정의

| 컬럼명 | 타입 | NULL | 기본값 | 설명 |
|--------|------|------|--------|------|
| identifier | String | NO | - | 이메일 등 식별자 |
| token | String @unique | NO | - | 인증 토큰 |
| expires | DateTime | NO | - | 토큰 만료 일시 |

#### 인덱스

| 인덱스명 | 컬럼 | 타입 | 용도 |
|----------|------|------|------|
| verification_tokens_identifier_token | identifier, token | UNIQUE | 토큰 조회 |
...
| stock | Int @default(0) | NO | 0 | 총 재고 (옵션 합산 동기화) |
| lowStockThreshold | Int @default(10) | NO | 10 | 저재고 알림 임계값 (상품별 설정) |
...
| productId | String | NO | - | FK → Product (CASCADE) |
| optionId | String? | YES | null | FK → ProductOption (옵션 없는 상품은 null) |
...
| saleDiscountAmount | Int @default(0) | NO | 0 | 상품 세일 할인 금액 |
| couponDiscountAmount | Int @default(0) | NO | 0 | 쿠폰 할인 금액 |
| discountAmount | Int @default(0) | NO | 0 | 총 할인 금액 (세일+쿠폰) |
| shippingFee | Int @default(0) | NO | 0 | 배송비 |
| pointUsed | Int @default(0) | NO | 0 | 사용 포인트 |
| finalAmount | Int | NO | - | 최종 결제 금액 |
| pointEarned | Int @default(0) | NO | 0 | 구매 확정 시 적립 포인트 |
| recipientName | String | NO | - | 수령인 |
...
| expiresAt | DateTime? | YES | null | PENDING 상태 만료 시각 |
| createdAt | DateTime @default(now()) | NO | now | |
| updatedAt | DateTime @updatedAt | NO | auto | |
...
| 참조 테이블 | 관계 | FK 컬럼 | 삭제 규칙 |
|------------|------|---------|-----------|
| OrderItem | 1:N | OrderItem.orderId | CASCADE |
| Review | 1:N | Review.orderId | CASCADE |
| Payment | 1:N | Payment.orderId | CASCADE |
| Refund | 1:N | Refund.orderId | CASCADE |
| Coupon | N:1 | couponId | SET NULL |
...
| status | OrderItemStatus @default(ORDERED) | NO | ORDERED | ORDERED, CANCELLED, RETURN_REQUEST, RETURNED, EXCHANGE_REQUEST, EXCHANGED |
| cancelReason | String? | YES | null | 취소 사유 |
| cancelledAt | DateTime? | YES | null | 취소 일시 |
| returnReason | String? | YES | null | 반품 사유 |
| returnRequestedAt | DateTime? | YES | null | 반품 신청 일시 |
| returnedAt | DateTime? | YES | null | 반품 완료 일시 |
| exchangeReason | String? | YES | null | 교환 사유 |
| exchangeProductId | String? | YES | null | 교환 대상 상품 ID |
| exchangeOptionId | String? | YES | null | 교환 대상 옵션 ID |
| exchangedAt | DateTime? | YES | null | 교환 완료 일시 |
| refundAmount | Int @default(0) | NO | 0 | 환불 금액 |
| createdAt | DateTime @default(now()) | NO | now | |
...
### 2.14b CouponTemplate (쿠폰 정책)

| 항목 | 내용 |
|------|------|
| **테이블명** | coupon_templates |
| **설명** | 관리자가 생성하는 쿠폰 정책 (정률/정액, 조건, 수량 제한) |
...
| validDays | Int @default(30) | NO | 30 | 발급 후 유효 기간 (일) |
| totalQuantity | Int? | YES | null | 총 발급 가능 수량 (null이면 무제한) |
| issuedCount | Int @default(0) | NO | 0 | 현재까지 발급 수량 |
| isActive | Boolean @default(true) | NO | true | 활성 여부 |
| description | String? | YES | null | 쿠폰 설명 |
| createdAt | DateTime @default(now()) | NO | now | |
| updatedAt | DateTime @updatedAt | NO | auto | |
...
| 참조 테이블 | 관계 | FK 컬럼 | 삭제 규칙 |
|------------|------|---------|-----------|
| Coupon | 1:N | Coupon.templateId | SET NULL |
...
|--------|------|------|--------|------|
| id | String @id @default(cuid()) | NO | cuid | PK |
| templateId | String? | YES | null | FK → CouponTemplate |
| userId | String | NO | - | FK → User (CASCADE) |
| name | String | NO | - | 쿠폰명 |
...
| 컬럼명 | 타입 | 설명 |
|--------|------|------|
| id, title, content(@db.Text), isPinned, isDeleted, viewCount, createdAt, updatedAt | | |
...
| relatedOrderId | String? | YES | null | 관련 주문 ID |
| createdBy | String | NO | - | FK → User (수행자, 관리자 또는 시스템) |
| createdAt | DateTime @default(now()) | NO | now | |
...
| 참조 테이블 | 관계 | FK 컬럼 | 삭제 규칙 |
|------------|------|---------|-----------|
| Product | N:1 | productId | CASCADE |
| ProductOption | N:1 | optionId | SET NULL |
| User | N:1 | createdBy | RESTRICT |
...
### 2.23 Payment (결제 트랜잭션)

| 항목 | 내용 |
|------|------|
| **테이블명** | payments |
| **설명** | 결제 시도/성공/실패/환불 트랜잭션 이력 |
| **예상 레코드 수** | 600K+ |
...
| id | String @id @default(cuid()) | NO | cuid | PK |
| orderId | String | NO | - | FK → Order |
| paymentMethod | PaymentMethod | NO | - | 결제 수단 |
| amount | Int | NO | - | 결제 금액 |
| status | PaymentStatus @default(PENDING) | NO | PENDING | 결제 상태 |
| paymentKey | String? | YES | null | PG사 결제 키 |
| transactionId | String? | YES | null | PG사 트랜잭션 ID |
| receiptUrl | String? | YES | null | 영수증 URL |
| failReason | String? | YES | null | 결제 실패 사유 |
| virtualAccountBank | String? | YES | null | 가상계좌 은행명 |
| virtualAccountNumber | String? | YES | null | 가상계좌 번호 |
| virtualAccountHolder | String? | YES | null | 가상계좌 예금주 |
| virtualAccountExpiry | DateTime? | YES | null | 가상계좌 입금 기한 |
| paidAt | DateTime? | YES | null | 결제 완료 일시 |
| createdAt | DateTime @default(now()) | NO | now | |
| updatedAt | DateTime @updatedAt | NO | auto | |
...
| 인덱스명 | 컬럼 | 타입 | 용도 |
|----------|------|------|------|
| payments_orderId | orderId | INDEX | 주문별 결제 조회 |
| payments_paymentKey | paymentKey | INDEX | PG 키 조회 |
| payments_status | status | INDEX | 상태별 조회 |
...
| id | String @id @default(cuid()) | NO | cuid | PK |
| orderId | String | NO | - | FK → Order |
| paymentId | String | NO | - | FK → Payment |
| amount | Int | NO | - | 환불 금액 |
| reason | String | NO | - | 환불 사유 |
| status | RefundStatus @default(PENDING) | NO | PENDING | PENDING, COMPLETED, FAILED |
| refundKey | String? | YES | null | PG 환불 트랜잭션 키 |
| refundMethod | String? | YES | null | 환불 수단 |
| refundedAt | DateTime? | YES | null | 환불 완료 일시 |
| createdAt | DateTime @default(now()) | NO | now | |
| updatedAt | DateTime @updatedAt | NO | auto | |
...
| 인덱스명 | 컬럼 | 타입 | 용도 |
|----------|------|------|------|
| refunds_orderId | orderId | INDEX | 주문별 환불 조회 |
| refunds_paymentId | paymentId | INDEX | 결제별 환불 조회 |
| refunds_status | status | INDEX | 상태별 조회 |
...
enum NotificationType { ORDER_STATUS COMMENT COMMENT_REPLY LIKE COUPON POINT SYSTEM EVENT LOW_STOCK }
...
enum StockChangeType { IN OUT ADJUST RETURN CANCEL }
enum PaymentStatus { PENDING COMPLETED FAILED CANCELLED PARTIAL_REFUNDED REFUNDED }
enum RefundStatus { PENDING COMPLETED FAILED }
enum OrderItemStatus { ORDERED CANCELLED RETURN_REQUEST RETURNED EXCHANGE_REQUEST EXCHANGED }
```
